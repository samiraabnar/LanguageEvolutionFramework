...
class SingleItemCommunicationEnv(object):
    def __init__(self,exp):
        ...
        self.talker_model_params["learning_rate"] = 0.01
    ...
    def game(self, number_of_epochs=400, cont=False):
        ...
        for e in np.arange(number_of_epochs):
            ...
            self.listener_costs.append(np.mean(listener_costs_epoch))
            self.talker_costs.append(np.mean(talker_costs_epoch))
            self.success_rates.append(np.sum(communication_success) / len(communication_success))
...
if __name__ == '__main__':
    ...
    print("Play ...")
    env.game(exp.number_of_epochs)
    listener_costs, talker_costs, success_rates = env.test_play_with_no_feedback()

    print(listener_costs, talker_costs, success_rates)
    ...
    env.game(exp.number_of_epochs,cont=True)
    listener_costs, talker_costs, success_rates = env.test_play_with_no_feedback()
