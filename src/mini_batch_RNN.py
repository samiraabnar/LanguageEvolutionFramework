...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        self.h0.name = 'h0'
        h0_init = np.zeros((n_hidden,), dtype="float32")
        param_idx += n_hidden

        self.bh = self.theta[param_idx:(param_idx + n_hidden)]
        self.bh.name = 'bh'
        bh_init = np.zeros((n_hidden,), dtype="float32")
        param_idx += n_hidden

        self.by = self.theta[param_idx:(param_idx + n_out)]
        self.by.name = 'by'
        by_init = np.zeros((n_out,), dtype="float32")
        param_idx += n_out
        ...
        self.theta_update = theano.shared(
            value=np.zeros(theta_shape, dtype="float32"))
...
class MetaRNN(BaseEstimator):
    ...
    def ready(self):
        ...
        if self.output_type == 'real':
            self.y = T.tensor3(name='y', dtype="float32")
        elif self.output_type == 'binary':
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        l_r = T.scalar('l_r', dtype="float32")
        mom = T.scalar('mom', dtype="float32")  # momentum
        ...
        if optimizer == 'sgd':
            ...
            while (epoch < self.n_epochs):
                ...
                for minibatch_idx in np.arange(n_train_batches):
                    ...
                    if iter % validation_frequency == 0:
                        ...
                        train_losses = [compute_train_error(i, n_train)
                                        for i in np.arange(n_train_batches)]
                        ...
                        if self.interactive:
                            test_losses = [compute_test_error(i, n_test)
                                            for i in np.arange(n_test_batches)]
...
def test_real(n_epochs=1000):
    ...
    model.fit(np.asarray(seq,dtype="float32"), np.asarray(targets,dtype="float32"), validate_every=100, optimizer='bfgs')
...
def test_binary(multiple_out=False, n_epochs=1000, optimizer='cg'):
    ...
    for seq_num in seqs:
        ...
        ax1.set_title('input')
        ax2 = plt.subplot(212)
        true_targets = plt.step(np.arange(n_steps), targets[:, seq_num, :],
                                marker='o')

        guess = model.predict_proba(seq[:, seq_num, :][:, np.newaxis, :])
        guessed_targets = plt.step(np.arange(n_steps), guess.squeeze())
